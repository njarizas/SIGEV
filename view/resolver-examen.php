...
<hr>
<noscript>
	<p class="text-danger">Debe habilitar el JavaScript en su navegador!!!
	</p>
</noscript>
<?php
	$idExamen = 0;
...
<div class="row">
	<div
		class="col-xs-10 col-xs-offset-1 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
                <?php
		if ($resultadosExamenesDAO->existeResultadoParaUsuarioYExamen ($estudiante,$idExamen)) {
			
			$examenes=$resultadosExamenesDAO->listarExamenPorIdUsuarioYExamen($idExamen, $estudiante);
		...
		foreach ( $p as $respuestaPreguna ) {
					...
					$idExamen= $tmp [0];
					$estudiante = $tmp [1];
					$resultadosPreguntasDAO->actualizar ( new ResultadoPregunta ( $idResultadoExamen, $idPregunta, $idRespuesta ) );
				}
				echo "<script>swal(\"Actualizaci�n exit�sa\", \"Su respuesta al examen ha sido actualizada \", \"success\");</script>";
			...
			if ($resultadosExamenesDAO->insertar ( new ResultadoExamen ( $estudiante, $idExamen, 0 ) ) > 0) {
				...
				echo "<script>swal(\"Error\", \"Ocurrió un error al tratar de registrar su respuesta del examen por favor comuníquese" . " con el administrador \", \"error\");</script>";
			}
		}
		echo '</div></div>';
	}
	if (isset ( $_POST ['examen'] ) || isset ( $_POST ['responder'] )) {
		...
		?>
                <form id="form_responder_examen" method="post"
			class="form-horizontal" action="resolver-examen.php">
		...
		?>
						...
						<button type="submit"
							class="btn btn-primary col-xs-8 col-xs-offset-2 col-sm-6 col-sm-offset-3 col-md-4 col-md-offset-4"
							id="responder" name="responder" value="responder">Guardar y
							Enviar</button>
					</div>
                            <?php
	}
